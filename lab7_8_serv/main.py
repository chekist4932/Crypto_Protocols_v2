...
def load_pkcs_elgamal(user_sign, center_sec_key, center_pub_key):
    ...
    set_of_AttributeValue = {"UserSignature": user_sign.hex(),
                             "Timestamp": timestamp,
                             "CenterSignature": center_signature,
                             "CenterSubjectPublicKeyInfo": {"SubjectPublicKeyInfo": {"alpha": center_pub_key["alpha"],
                                                                                     "beta": center_pub_key["beta"],
                                                                                     "p": center_pub_key["p"]},
                                                            "PKCS10CertRequest": "NULL", "Certificate": "NULL",
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM, proto=0) as serv_sock:
    serv_sock.bind(("10.0.126.164", 50228))
    print(serv_sock)
    serv_sock.listen(4)
                ...
                if list(dict_for_check)[0] == "CMSVersion":
                    try:
                        ...
                        if user_type_of_encrypt == "ELGAMALdsi":
                            ...
                            # Алгоритм подписи
                            center_pub_key, center_sek_key, user_pub_key, user_mod_n = get_keys(user_type_of_encrypt,
                                                                                                dict_for_check)
                        ...
                        if check_flag == True:
                            if user_type_of_encrypt == "ELGAMALdsi":
                                ref_user_signature = hashing(user_msg + json.dumps(user_signature).encode().hex(),
                                                             user_hash_type)
