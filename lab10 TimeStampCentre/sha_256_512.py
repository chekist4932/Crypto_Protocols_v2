...
def sig_0_256(x: int) -> int:
    return int(right_rotate(x, 7, 32), 2) ^ int(right_rotate(x, 18, 32), 2) ^ (x >> 3)
...
def sig_1_256(x: int) -> int:
    return int(right_rotate(x, 17, 32), 2) ^ int(right_rotate(x, 19, 32), 2) ^ (x >> 10)
...
def hex_to_bin_v3(mess_bytes: bytes, choose) -> int or str:
    if choose == 1:
