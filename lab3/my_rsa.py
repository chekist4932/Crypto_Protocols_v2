...
class RSA:
    ...
    def encrypt(pub_key: list[int, int], bytes_mess: bytes):
        ...
        _encrypted_msg_blocks = [pow(int.from_bytes(_block, byteorder='big'), pub_key[1], pub_key[0]) for _block in _msg_blocks.copy()]
    ...
    def decrypt(sec_key: list[int, int, int], encrypted_msg: bytes):
        ...
        for _block_in_decrypted in range(len(decrypted_msg)):
            _byte_check = decrypted_msg[_block_in_decrypted][-1]
            __checker = 0
            for i in range(len(decrypted_msg[_block_in_decrypted]) - 1, (_block_len - _byte_check) - 1, -1):
                if decrypted_msg[_block_in_decrypted][i] == _byte_check:
                    __checker += 1

            if __checker == _byte_check:
                decrypted_msg[_block_in_decrypted] = decrypted_msg[_block_in_decrypted][:_block_len - __checker]
            else:
