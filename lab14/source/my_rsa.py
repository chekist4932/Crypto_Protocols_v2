...
class RSA:
    ...
    @staticmethod
    def encrypt(pub_key: dict, bytes_mess: bytes):

        _msg_len_bytes = len(bytes_mess)
        _key_byte_len = len(pub_key["N"].to_bytes(math.ceil(math.log2(pub_key["N"]) / 8), byteorder="big"))
        ...
        _encrypted_msg_blocks = [pow(int.from_bytes(_block, byteorder='big'), pub_key["publicExponent"], pub_key["N"])
                                 for _block in
                                 _msg_blocks.copy()]
    ...
    @staticmethod
    def decrypt(sec_key: dict, encrypted_msg: bytes):

        _msg_len_bytes = len(encrypted_msg)
        _key_byte_len = len(
            (sec_key["prime1"] * sec_key["prime2"]).to_bytes(
                math.ceil(math.log2((sec_key["prime1"] * sec_key["prime2"])) / 8), byteorder="big"))
        ...
        _decrypted_msg = [x.to_bytes(_block_len, byteorder="big") for x in
                          [pow(num, sec_key['privateExponent'], sec_key["prime1"] * sec_key["prime2"]) for num in _encrypted_msg_blocks]]
